              ...
              {data.map((item: OrderType)=>(
                <tr className="text-sm md:text-base bg-red-200 " key={item.id}>
                  ...
                  {session?.user.isAdmin ? (
                    <td>
                      <form className='flex items-center justify-center gap-10 px-10' onSubmit={(e) => handleUpdate(e, item.id)}>
                        <input placeholder={item.status} className='p-2 ring-1 ring-red-100 rounded-md w-[75%]'/>
